...
pub enum Error {
    ...
    /// Returned by the [`Connection`](crate::Connection) [`Stream`](futures_util::stream::Stream) when receiving a new stream failed.
    #[error("Error on receiving a new stream: {0}")]
    IncomingStream(ConnectionError),
    /// Returned by the [`Client::connect`](crate::Client::connect) if establishing a connection to the given `address` failed.
    #[error("Error on establishing a connection to a remote address: {0}")]
    Connect(ConnectError),
    /// Returned by the [`Client::connect`](crate::Client::connect) if connecting to the remote `address` failed.
